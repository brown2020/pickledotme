...
const WORD_LISTS = {
  easy: [
    "DILL", "BRINE", "SOUR", "JAR", "SALT", "SNAP", "VINE", "SEED", 
    "CHIP", "BITE", "ZEST", "TANG", "CUKE", "FARM", "PICK"
  ],
  medium: [
    "PICKLE", "GHERKIN", "GARLIC", "FLAVOR", "CRUNCH", "SNACKS", 
    "RECIPE", "BARREL", "FERMENT", "HARVEST", "GARDEN", "RELISH",
    "KOSHER", "SLICED", "SPEARS"
  ],
  hard: [
    "CUCUMBER", "VINEGAR", "PRESERVED", "DELICIOUS", "CRUNCHY", 
    "APPETIZER", "HOMEMADE", "NATURALLY", "BRINING", "SEASONED",
    "SANDWICH", "NUTRITION"
  ],
};
...
export function useWordGame() {
  ...
  const selectLetter = useCallback((letter: Letter) => {
  ...
  const deselectLetter = useCallback((letter: Letter) => {
  ...
  const checkWord = useCallback(() => {
    ...
    if (selectedLetters.length !== currentWord.length || currentWord.length === 0) return;
  ...
  }, [selectedLetters, currentWord, timeLeft, hintUsed, gameBase, wordsCompleted, generateNewWord, isProcessing, showCorrect]);

  // Trigger word check when selection changes
  useEffect(() => {
    if (selectedLetters.length === currentWord.length && currentWord.length > 0 && !isProcessing && !showCorrect) {
      checkWord();
    }
  }, [selectedLetters.length, currentWord.length, checkWord, isProcessing, showCorrect]);
